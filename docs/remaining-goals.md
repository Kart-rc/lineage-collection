...
|---|---|
| Status | Active |
| Branch | `codex/lineage-prototype` |
...
The public-origin policy and public-address resolver are complete. The actual bounded remote Git
process, exact checkout, cleanup, and dependency wiring remain.

**Remaining implementation**

- resolve and revalidate all network destinations immediately before connection;
- pin validated public addresses for Git/libcurl and disable redirects and ambient proxies;
- run a trusted absolute Git executable with isolated configuration, no hooks/templates/credential
  helpers, no interactive prompts, no inherited secrets, and no shell;
- stream and cap stdout/stderr, enforce an overall deadline, and terminate the complete process group
  through TERM, bounded group probing, and KILL escalation;
- fetch only the requested exact 40-character commit into a private temporary checkout;
- verify origin, exact HEAD, clean tracked state, path/blob/count limits, and unsupported symlink or
  submodule conditions through the hardened local snapshot reader;
- materialize the immutable snapshot before guaranteed cleanup on success and every failure path;
- wire the local and remote providers to `RepositoryAcquisitionService` and
  `RepositoryCollectionService` without concrete infrastructure imports in the application layer.
